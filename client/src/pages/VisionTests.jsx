...
export default function VisionTests() {
  ...
  const fetchTests = async () => {
    try {
      const response = await api.get('/vision-tests');
    ...
    try {
      if (isEditing) {
        ...
        try {
          ...
          setAiError(aiErr.response?.data?.error || 'Failed to analyze vision test');
        } finally {
          setAiLoading(false);
        }
      }
  ...
  const handleAIAnalyze = async () => {
    ...
    try {
      ...
      setAiError(error.response?.data?.error || 'Failed to analyze vision test');
    } finally {
      setAiLoading(false);
    }
  };
